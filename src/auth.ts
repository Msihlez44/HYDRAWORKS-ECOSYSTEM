...
import { z } from 'zod';
export const registerSchema=z.object({firstName:z.string().min(2).max(60),lastName:z.string().min(2).max(60),email:z.string().email(),phone:z.string().min(9).max(20),password:z.string().min(10).regex(/[A-Z]/).regex(/[a-z]/).regex(/[0-9]/),role:z.enum(['CUSTOMER','BUSINESS_OWNER','DRIVER','SERVICE_PROVIDER','WORKER','SUPPLIER']).default('CUSTOMER')});
